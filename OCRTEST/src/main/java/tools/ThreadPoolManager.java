...
public class ThreadPoolManager {
	...
	public static ThreadPoolManager getInstance(int threadN) {
		synchronized(ThreadPoolManager.class) {
			if(instance==null) {
				instance = new ThreadPoolManager(threadN);
			}else if(threadNum!=threadN){
				//线程数不相等则重新创建
				instance = new ThreadPoolManager(threadN);
			}
		}
	...
	private ThreadPoolManager(int num) {
		if(num<1) {
			...
			threadNum = num;
			threadPool = new ThreadPoolExecutor(num, num*10, 1000, TimeUnit.SECONDS, new LinkedBlockingDeque<Runnable>());
		}
	}
